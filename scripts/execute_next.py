...
def choose(actions):
    for a in actions:
        ...
        if not a.get('executable_now', False): continue
...
def run_action(action, workdir):
    ...
    if float(action.get('risk_level', 10)) > THRESHOLD:
        r['blocker_reason'] = 'risk above auto threshold'
        return r
    et = action.get('execution_type')
    if et == 'command':
        p = subprocess.run(
            action.get('command_or_patch', ''),
        ...
        p = subprocess.run(
            ['python3', '-c', action.get('command_or_patch', '')],
...
def main(queue_path, result_path, workdir):
    ...
    if not action:
        save_json(result_path, {
            'status': 'idle',
            'reason': 'no executable pending action',
            'timestamp': now_iso()
        })
        return
